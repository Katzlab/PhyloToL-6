...
#Example script call: python ContaminationBySisters_v2.0.py --input /Path/to/trees
...
def get_args():
	...
	parser = argparse.ArgumentParser(
		prog = 'Contamination by Sisters, Version 2.0',
		description = "Updated March 31st, 2023 by Auden Cote-L'Heureux."
	)

	parser.add_argument('-i', '--input', type = str, help = 'Path to the folder containing the input trees')
	parser.add_argument('-l', '--level', choices = ['major', 'minor', 'species'], default = 'major', help = 'The level at which to report sister relationships. Options are major, minor, and species.')
...
def write_all_data(args):
	...
	report = open('report_walk_contamination_single-all.csv', 'w')
...
def summarize(args):
	...
	report = open('report_walk_contamination_single-all.csv', 'r').readlines()[1:]
	output = open('summary_contamination_all.csv', 'w')
	...
	for line in report:
		...
		if (args.branch_length_filter and float(line[5]) < float(line[6])) or (not args.branch_length_filter):
			...
			if(not args.single_sister_only and len(sister_taxa) > 1):
	...
	output.write('Taxon,SameMinor,NM,Total,PropSM,' + ','.join(sisters_to_write) + '\n')

	#Write out the counts for sister relationships
	for taxon in sisters_by_taxon:
		output.write(taxon + ',' + str(stats_by_taxon[taxon]['same']) + ',' + str(stats_by_taxon[taxon]['nm']) + ',' + str(stats_by_taxon[taxon]['total']) + ',' + str(stats_by_taxon[taxon]['same']/stats_by_taxon[taxon]['total']) + ',')

		for sister in sisters_to_write:
