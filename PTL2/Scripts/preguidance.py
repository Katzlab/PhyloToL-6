...
def run(params):
	...
	of_log = open(params.output + '/Output/Temp/OF-SF_Diamond/OF_LOG.tsv', 'w')
	of_log.write('OG\tTaxon\tSequence\tMaster\tID\tAlignmentLength\tGaps\tMasterLength\n')
					...
					while flag == 0:
						...
						for line in diamond_out:
							...
							if ((alignment_length - gaps) < params.overlap_cutoff * len(recs[0].seq) and cycle == 0) or identity > params.sim_cutoff:
								...
								of_log.write(og + '\t' + taxon_file[:10] + '\t' + seq + '\t' + recs[0].id + '\t' + str(identity) + '\t' + str(alignment_length) + '\t' + str(gaps) + '\t' + str(len(recs[0].seq)) + '\n')
